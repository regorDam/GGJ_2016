...
Shader "GGJ16/Toon + UserColor + Outline + Alpha" {
    Properties {
        ...
        _UserColor ("User Color", Color) = (1,1,1,1)
...
////// Lighting:
                ...
                float3 finalColor = ((((lerp( lerp( lerp( _MainTex_var.rgb, _UserColor.rgb, _ColorMap_var.rgb.r ), _UserColor.rgb, _ColorMap_var.rgb.g ), _UserColor.rgb, _ColorMap_var.rgb.b ))*UNITY_LIGHTMODEL_AMBIENT.rgb)*lerp(_RampShadow.rgb,_RampLight.rgb,_RampMap_var.rgb)*(_LightColor0.rgb*attenuation))+(pow(1.0-max(0,dot(normalDirection, viewDirection)),exp2(_FresnelPower))*_FresnelColor.rgb*_FresnelColor.a));
